...
Tray::Tray(QWidget* parent) : QSystemTrayIcon(parent) {
  ...
  connect(this, SIGNAL(activated(QSystemTrayIcon::ActivationReason)), this,
          SLOT(trayActivated(const QSystemTrayIcon::ActivationReason)));
  this->show();
  menu = new QMenu();
  ...
  AppMsgHandler::getInstance().regSignal("/window/hide", app_hide,
                                         SIGNAL(triggered()));
  menu->addAction(app_hide);

  app_reset_geo = new QAction(QString("reset geometry"));
  AppMsgHandler::getInstance().regSignal("/window/reset_geometry",
                                         app_reset_geo, SIGNAL(triggered()));
