...
AppMsgHandler::AppMsgHandler() : QObject(nullptr) {}

AppMsgHandler::~AppMsgHandler() {}

void AppMsgHandler::regSignal(const SID_t &sid, QObject *obj,
                              const char *method) {
  ...
  for (auto sig_info = curr_tuple.vsig_infos.constBegin();
       sig_info != curr_tuple.vsig_infos.constEnd(); sig_info++)
  ...
  // look for slots
  for (auto slot_info = curr_tuple.vslot_infos.begin();
       slot_info != curr_tuple.vslot_infos.end(); slot_info++)
...
void AppMsgHandler::bindSlot(const SID_t &sid, QObject *obj, const char *method,
                             const Qt::ConnectionType c_type) {
  std::lock_guard guard(_hc_lock);
  if (_hash_connect.find(sid) == _hash_connect.end()) {
    SigTuple new_tuple;
    _hash_connect.insert(sid, new_tuple);
  ...
  auto &curr_tuple = _hash_connect[sid];
  for (auto slot_info = curr_tuple.vslot_infos.constBegin();
       slot_info != curr_tuple.vslot_infos.constEnd(); slot_info++)
  ...
  // look for sigs
  for (auto sig_info = curr_tuple.vsig_infos.begin();
       sig_info != curr_tuple.vsig_infos.end(); sig_info++)
...
bool AppMsgHandler::postEvent(const SID_t &sid, QEvent *ev,
                              Qt::EventPriority pri) {
  auto ev_iter = m_ev_listener.find(sid);
...
bool AppMsgHandler::sendEvent(const SID_t &sid, QEvent *ev) {
  ...
  if (ti_iter == m_ev_listener.end()) return false;
