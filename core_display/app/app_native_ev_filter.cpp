...
#include "Windows.h"
#include "windowsx.h"
const UINT AppNativeEvFilter::WM_IONAGL_LOOKAT =
    RegisterWindowMessage(L"WM_IONAGL_LOOKAT");
#endif

AppNativeEvFilter::AppNativeEvFilter() : QAbstractNativeEventFilter() {
  AppMsgHandler::getInstance().regSignal(
      "/mouse", this,
      SIGNAL(sendGlMouseEvent(const QEvent::Type, const Qt::MouseButton,
                              const QPoint)));
}

bool AppNativeEvFilter::nativeEventFilter(const QByteArray &eventType,
