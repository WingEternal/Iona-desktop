...
AppParam::AppParam() : instance(new AppParamWorker), thrd(new QThread) {
...
AppParamWorker::AppParamWorker() : QObject(nullptr) {
  // Read ini
  ...
  for (auto it = default_value.cbegin(); it != default_value.cend(); it++)
    if (!settings->contains(it.key())) settings->setValue(it.key(), it.value());

  // Bind slot
  AppMsgHandler::getInstance().bindSlot(
      "/app/set_param", this, SLOT(setParam(const QString&, const QVariant&)),
      Qt::QueuedConnection);
}
