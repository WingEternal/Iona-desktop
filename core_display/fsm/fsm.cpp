...
void FSM::register_ss()
{
    ...
    handler.regSignal("/voice/play", this,
        SIGNAL(voicePlay(const QUrl)));
    handler.bindSlot("/voice/playback_state", this,
        SLOT(onPlayerStateChanged(const QMediaPlayer::PlaybackState)));

    handler.regSignal("/animate/task", this, SIGNAL(sendGLObjTask(const QString, const QVariant, const double, const QEasingCurve::Type)));
    handler.bindSlot("/animate/task/finished", this, SLOT(onGLObjTaskFinished(const QString&)));
    ...
    handler.bindSlot("/mouse", this,
        SLOT(mouseEventDispatch(const QEvent::Type, const Qt::MouseButton, const QPoint)));
}
...
void FSM::onStart()
{
    ...
    AppMsgHandler::getInstance().regSignal("/animate/update", _fsm_loop, SIGNAL(timeout()));
    _fsm_elapsed = new QElapsedTimer;
    AppParam::getInstance().setParam("D$clock", _fsm_clock);
    AppParam::getInstance().setParam("D$clock/delta", (_fsm_clock - _fsm_last_update_clock) / 1000.0);
...
void FSM::mouseEventDispatch(const QEvent::Type type, const Qt::MouseButton b, const QPoint pt)
{
...
//        static constexpr double focus_px_range_square = focus_px_range * focus_px_range;
//        if(pow(pt.x() - widget_center.x(), 2) + pow(pt.y() - widget_center.y(), 2) <= focus_px_range_square)
...
void FSM::loopStateMachine()
{
    ...
    case START:
    {
        ...
        if(!(_buffer_states & 0x1)
            ...
            && this->isSignalConnected
                (QMetaMethod::fromSignal(&FSM::voicePlay)))
        {
