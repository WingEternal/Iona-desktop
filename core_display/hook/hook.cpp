...
BOOL Hook::UnInstallMouseHook() { return UnhookWindowsHookEx(m_hook); }

BOOL Hook::InstallKeybordHook() {
  kb_hook =
      SetWindowsHookEx(WH_KEYBOARD_LL, keyBordProc, GetModuleHandle(NULL), 0);
  if (kb_hook != nullptr)
    return TRUE;
  else
    return FALSE;
...
BOOL Hook::UnInstallKeybordHook() { return UnhookWindowsHookEx(kb_hook); }
...
LRESULT CALLBACK Hook::MouseProc(int nCode, WPARAM wParam, LPARAM lParam) {
  if (wParam == WM_LBUTTONDOWN || wParam == WM_MOUSEMOVE ||
    ...
    switch (wParam) {
      case WM_LBUTTONDOWN:
        PostMessage(hwnd, WM_IONAGL_LOOKAT, WM_LBUTTONDOWN,
                    MAKELPARAM(mpt.x, mpt.y));
        break;
      case WM_MOUSEMOVE:
        PostMessage(hwnd, WM_IONAGL_LOOKAT, WM_MOUSEMOVE,
                    MAKELPARAM(mpt.x, mpt.y));
        break;
      case WM_LBUTTONUP:
        PostMessage(hwnd, WM_IONAGL_LOOKAT, WM_LBUTTONUP,
                    MAKELPARAM(mpt.x, mpt.y));
        break;
      default:
        break;
...
LRESULT CALLBACK Hook::keyBordProc(int nCode, WPARAM wParam, LPARAM lParam) {
  if (wParam == WM_KEYDOWN) {
    ...
    PostMessage(hwnd, WM_KEYDOWN, keyhookstruct->vkCode,
