...
/* DO NOT EDIT */
// Ring circumference
constexpr GLfloat ring_tb_perimeter =
    GLObj_DataRing::ring_tb_height * 17650 / 540;
constexpr GLfloat ring_tbf_perimeter =
    GLObj_DataRing::ring_tbf_height * 17650 / 540;
constexpr GLfloat ring_m_perimeter =
    GLObj_DataRing::ring_m_height * 18564 / 540;
...
constexpr GLint ring_tb_vertices_size =
    2 * 5 * (GLObj_DataRing::ring_tb_res + 1);
constexpr GLint ring_tbf_vertices_size =
    2 * 5 * (GLObj_DataRing::ring_tbf_res + 1);
...
constexpr void GLObj_DataRing::CompileVertices(GLfloat *container_array,
                                               const GLint size,
...
void GLObj_DataRing::initializeModel() {
  // Ring_tb
  CompileVertices(vertices_ring_tb, ring_tb_vertices_size, ring_tb_radius,
                  ring_tb_height, ring_tb_res);
  vao_ring_tb =
      loadModel(sprogram_mix2, vertices_ring_tb, sizeof(vertices_ring_tb));

  // Ring_tbf
  CompileVertices(vertices_ring_tbf, ring_tbf_vertices_size, ring_tbf_radius,
                  ring_tbf_height, ring_tbf_res);
  vao_ring_tbf =
      loadModel(sprogram_general, vertices_ring_tbf, sizeof(vertices_ring_tbf));

  // Ring_m
  CompileVertices(vertices_ring_m, ring_m_vertices_size, ring_m_radius,
                  ring_m_height, ring_m_res);
  vao_ring_m = loadModel(sprogram_rm, vertices_ring_m, sizeof(vertices_ring_m));
}

QOpenGLVertexArrayObject *GLObj_DataRing::loadModel(
    QOpenGLShaderProgram *sp, const GLfloat *vertices,
  ...
  sp->setAttributeBuffer(attr, GL_FLOAT, 3 * sizeof(GLfloat), 2,
                         5 * sizeof(GLfloat));
  sp->enableAttributeArray(attr);
  // Clean
  vbo.release();
