...
class GLObj_DataRing : public GLObjectBase {
 ...
 public slots:
  ...
  void addTask(const QString& task_name, const QVariant& target_state,
               const double interval,
               const QEasingCurve::Type curve = QEasingCurve::Linear);

 private:
  // Generate model at compile time
  constexpr void CompileVertices(GLfloat* container_array, const GLint size,
                                 const GLfloat radius, const GLfloat height,
                                 const GLint res);
  ...
  QOpenGLVertexArrayObject* loadModel(QOpenGLShaderProgram* sp,
  ...
  template <typename T>
  void addTaskPrivate(QPropertyAnimation* _qpa, const T& start_state,
                      const QVariant& end_state, const double interval,
                      const QEasingCurve::Type curve) {
    if (_qpa->state() == QPropertyAnimation::Running) _qpa->pause();
