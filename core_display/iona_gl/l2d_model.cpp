...
void
L2dModel::LoadAssets(const Csm::csmChar* dir, const  Csm::csmChar* fileName)
{
    _modelHomeDir = dir;
    if (_debugMode)
        L2dPal::PrintLog("[APP]load model setting: %s", fileName);
    csmSizeInt size;
...
void
L2dModel::SetupModel(ICubismModelSetting* setting)
{
    _updating = true;
    _initialized = false;
...
void L2dModel::PreloadMotionGroup(const csmChar* group)
{
    ...
    for (csmInt32 i = 0; i < count; i++)
    {
        ...
        if (_debugMode)
            L2dPal::PrintLog("[APP]load motion: %s => [%s_%d] ", path.GetRawString(), group, i);
    ...
    ACubismMotion::FinishedMotionCallback onFinishedMotionHandler)
{
    ...
    {
        if (_debugMode)
            L2dPal::PrintLog("[APP]can't start motion.");
...
void L2dModel::SetExpression(const csmChar* expressionID)
{
    ...
    if (_debugMode)
        L2dPal::PrintLog("[APP]expression: [%s]", expressionID);

    if (motion != nullptr)
        _expressionManager->StartMotionPriority(motion, false, L2dConfig::PriorityForce);
    else if (_debugMode)
...
void L2dModel::SetupTextures()
{
    for (csmInt32 modelTextureNumber = 0; modelTextureNumber < _modelSetting->GetTextureCount(); modelTextureNumber++)
    {
        ...
        L2dTexManager::TextureInfo* texture = _texManager->CreateTextureFromPngFile(texturePath.GetRawString());
