 ...
 * formula from
 * [fifbro](https://blog.csdn.net/qq_21950929/article/details/78989215)
 */
...
GLObj_DataRing::GLObj_DataRing(QOpenGLWidget* parent,
                               const QMatrix4x4& tf_camera_)
    : GLObjectBase(parent),
      tex_ring_tb{nullptr},
  ...
  AppParam::getInstance().getParam("S$animate/ring_m_animation_switch_by_frame",
                                   animate_switch_thrd);
  AppParam::getInstance().getParam("S$animate/ring_tb_roll_spd",
                                   ring_tb_roll_spd);

  AppMsgHandler& handler = AppMsgHandler::getInstance();
  handler.bindSlot("/animate/ring_scale", this,
                   SLOT(setRingScale(const float)));
  handler.bindSlot("/animate/ring_rpm", this, SLOT(setRingRpm(const float)));
  handler.bindSlot("/animate/ring_t_pos", this,
                   SLOT(setRingTPos(const QVector3D&)));
  handler.bindSlot("/animate/ring_m_pos", this,
                   SLOT(setRingMPos(const QVector3D&)));
  handler.bindSlot("/animate/ring_b_pos", this,
                   SLOT(setRingBPos(const QVector3D&)));
  handler.bindSlot("/animate/task", this,
                   SLOT(addTask(const QString&, const QVariant&, const double,
                                const QEasingCurve::Type)));
  handler.regSignal("/animate/task/finished", this,
                    SIGNAL(taskFinished(const QString)));
...
GLObj_DataRing::~GLObj_DataRing() {
  for (int i = 0; i < 3; i++) tex_ring_tb[i]->destroy();
  tex_ring_m_empty->destroy();
  for (int i = 0; i < 20; i++) tex_ring_m_active[i]->destroy();
}
...
QOpenGLTexture* GLObj_DataRing::loadTexture(const QString& address) {
  QOpenGLTexture* tex_ptr = new QOpenGLTexture(QImage(address).mirrored(),
...
void GLObj_DataRing::initializeTexture() {
  ...
  for (int i = 0; i < 20; i++)
    tex_ring_m_active[i] = loadTexture(":/charater/texture/ring_m_a" +
                                       QString::number(i) + "_559.png");
}
...
void GLObj_DataRing::loadGenernalShader() {
  ...
  attr_sp_general_Delta_Tex_S =
      sprogram_general->uniformLocation("Delta_Tex_S");
}
...
void GLObj_DataRing::loadRmShader() {
  ...
  if (!sprogram_rm->link()) qDebug() << "Rm Shader Program Link Failed";
...
void GLObj_DataRing::paint() {
  ...
  if (ring_spin_angle > 1) ring_spin_angle -= 1;

  // Update roll dist
  for (int i = 0; i < 2; i++) {
    if (random_stop_elapsed[i] <= 0) {
      ring_tb_roll_dist[i] += ring_tb_roll_spd * elapsed_time / 60.0f;
      if (ring_tb_roll_dist[i] > 1) ring_tb_roll_dist[i] -= 1;
...
void GLObj_DataRing::drawRingTB() {
  ...
  sprogram_mix2->setUniformValue(attr_sp_mix2_Delta1f_Tex_T,
                                 -ring_tb_roll_dist[0]);
  vao_ring_tb->bind();
  ...
  sprogram_mix2->setUniformValue(attr_sp_mix2_Transform,
                                 tf_camera * tf_ring_tb);
  glDrawArrays(GL_QUAD_STRIP, 0, 2 * (ring_tb_res + 1));

  // Draw ring_b
  sprogram_mix2->setUniformValue(attr_sp_mix2_Delta0f_Tex_S,
                                 -ring_spin_angle + 0.5f);
  sprogram_mix2->setUniformValue(attr_sp_mix2_Delta1f_Tex_T,
                                 -ring_tb_roll_dist[1] + 0.5f);
  tf_ring_tb.setToIdentity();
...
void GLObj_DataRing::drawRingM() {
  ...
  if (animate_switch_counter >= animate_switch_thrd) {
    ...
    if (ring_m_animate_counter >= 20) {
      ring_m_animate_counter = 0;
      ring_m_active_zone++;
      if (ring_m_active_zone >= 4) ring_m_active_zone = 0;
    }
  }
}

float GLObj_DataRing::RingScale() { return _ring_scale; }
float GLObj_DataRing::RingRpm() { return _ring_rpm; }
QVector3D GLObj_DataRing::RingTPos() { return _ring_t_posb; }
QVector3D GLObj_DataRing::RingMPos() { return _ring_m_posb; }
QVector3D GLObj_DataRing::RingBPos() { return _ring_b_posb; }

void GLObj_DataRing::setRingScale(const float scale) { _ring_scale = scale; }
void GLObj_DataRing::setRingRpm(const float rpm) { _ring_rpm = rpm; }
void GLObj_DataRing::setRingTPos(const QVector3D& pos) { _ring_t_posb = pos; }
void GLObj_DataRing::setRingMPos(const QVector3D& pos) { _ring_m_posb = pos; }
void GLObj_DataRing::setRingBPos(const QVector3D& pos) { _ring_b_posb = pos; }

void GLObj_DataRing::addTask(const QString& task_name,
                             const QVariant& target_state,
                             const double interval,
                             const QEasingCurve::Type curve) {
...
void GLObj_DataRing::execStartMotion() {
  QSequentialAnimationGroup* seqAnimationG1 =
  ...
  QSequentialAnimationGroup* seqAnimationG2 =
      new QSequentialAnimationGroup(this);
