...
L2dTexManager::~L2dTexManager() { ReleaseTextures(); }

L2dTexManager::TextureInfo* L2dTexManager::CreateTextureFromPngFile(
    std::string fileName) {
  // search loaded texture already.
  for (Csm::csmUint32 i = 0; i < _textures.GetSize(); i++) {
    if (_textures[i]->fileName == fileName) return _textures[i];
  }
...
void L2dTexManager::ReleaseTextures() {
  for (Csm::csmUint32 i = 0; i < _textures.GetSize(); i++) {
...
void L2dTexManager::ReleaseTexture(Csm::csmUint32 textureId) {
  for (Csm::csmUint32 i = 0; i < _textures.GetSize(); i++) {
    if (_textures[i]->id != textureId) continue;
    delete _textures[i];
    _textures.Remove(i);
    break;
  }
...
void L2dTexManager::ReleaseTexture(std::string fileName) {
  for (Csm::csmUint32 i = 0; i < _textures.GetSize(); i++) {
...
L2dTexManager::TextureInfo* L2dTexManager::GetTextureInfoById(
    GLuint textureId) const {
  for (Csm::csmUint32 i = 0; i < _textures.GetSize(); i++)
    if (_textures[i]->id == textureId) return _textures[i];
  return nullptr;
}
