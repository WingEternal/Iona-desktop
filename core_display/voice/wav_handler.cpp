...
WavHandler::~WavHandler() {
  if (_pcmData != NULL) ReleasePcmData();
}

void WavHandler::register_ss() {
  AppMsgHandler::getInstance().bindSlot("/voice/play", this,
                                        SLOT(play(const QUrl)));
  AppMsgHandler::getInstance().bindSlot("/animate/update", this,
                                        SLOT(update()));

  AppMsgHandler::getInstance().regSignal("/voice/rms", this,
                                         SIGNAL(publishRms(const double)));
  AppMsgHandler::getInstance().regSignal(
      "/voice/playback_state", _player,
      SIGNAL(playbackStateChanged(QMediaPlayer::PlaybackState)));

  connect(_player, SIGNAL(mediaStatusChanged(QMediaPlayer::MediaStatus)), this,
          SLOT(onPlayerMediaLoaded(const QMediaPlayer::MediaStatus)));
}

void WavHandler::update() {
  double rms = 0.0f;
  // Directly return if player is not playing
  if (this->_player->playbackState() != QMediaPlayer::PlayingState) return;
  ...
  goalOffset =
      static_cast<uint32_t>(_userTimeSeconds * _wavFileInfo._samplingRate);
  if (goalOffset > _wavFileInfo._samplesPerChannel) {
    goalOffset = _wavFileInfo._samplesPerChannel;
  }

  // RMS計測
  for (uint32_t channelCount = 0; channelCount < _wavFileInfo._numberOfChannels;
       channelCount++) {
    for (uint32_t sampleCount = _sampleOffset; sampleCount < goalOffset;
         sampleCount++) {
      float pcm = _pcmData[channelCount][sampleCount];
      rms += pcm * pcm;
    }
  }
  rms = fmax(0, sqrt(rms / (_wavFileInfo._numberOfChannels *
...
void WavHandler::play(const QUrl filePath) {
  ...
  if (!LoadWavFile(filePath.toLocalFile())) return;
}
...
bool WavHandler::LoadWavFile(const QString& filePath) {
  ...
  do {
    ...
    // 領域確保
    _pcmData = static_cast<float**>(
        malloc(sizeof(float*) * _wavFileInfo._numberOfChannels));
    for (uint32_t channelCount = 0;
         channelCount < _wavFileInfo._numberOfChannels; channelCount++)
      _pcmData[channelCount] = static_cast<float*>(
          malloc(sizeof(float) * _wavFileInfo._samplesPerChannel));
    // 波形データ取得
    for (uint32_t sampleCount = 0;
