 ...
 private:
  ...
  struct ByteReader {
    ...
    uint32_t Get24LittleEndian() {
      const uint32_t ret = (_fileByte[_readOffset + 2] << 16) |
    ...
    uint32_t Get32LittleEndian() {
      const uint32_t ret = (_fileByte[_readOffset + 3] << 24) |
    ...
    bool GetCheckSignature(const QString& reference) {
      ...
      for (uint32_t signatureOffset = 0; signatureOffset < 4;
           signatureOffset++) {
        getSignature[signatureOffset] = static_cast<char>(Get8());
      }
      return (getSignature[0] == reference[0]) &&
             (getSignature[1] == reference[1]) &&
             (getSignature[2] == reference[2]) &&
             (getSignature[3] == reference[3]);
    }
