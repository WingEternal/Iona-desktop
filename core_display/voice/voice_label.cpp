...
VoiceLabel::VoiceLabel(QWidget *parent) : QLabel(parent) {
  ...
  AppMsgHandler::getInstance().bindSlot(
      ...
      SLOT(onPlayerPlaybackStateChanged(QMediaPlayer::PlaybackState)));
  AppMsgHandler::getInstance().bindSlot("/voice/label_text", this,
                                        SLOT(setText(const QString &)));

  connect(label_animation, SIGNAL(finished()), this,
          SLOT(onAnimationFinished()));
}

VoiceLabel::~VoiceLabel() {}

void VoiceLabel::onPlayerPlaybackStateChanged(
