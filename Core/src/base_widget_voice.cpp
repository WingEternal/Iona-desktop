...
void BaseWidget::setupVoice()
{
    ...
    voice_label_ptr->setGeometry(260, 275, 220, 50);
...
void BaseWidget::Slot_Hitbox_Hit()
{
    ...
    {
    case 1:
        voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/Iona02_ptr->mp3"));
        voice_label_ptr->setText(QString("どうしてここにいるのが分かったの？提督。何か用？"));
        voice_label_ptr->show();
        break;
    case 2:
        voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/Iona03_ptr->mp3"));
...
void BaseWidget::Slot_VoiceTimer_Timeout()
{
    ...
    if(currTime.minute() == 0 && currTime.second() == 0 && voice_player_ptr->state() == QMediaPlayer::StoppedState)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/0200_ptr->mp3"));
            voice_label_ptr->setText("もしかして、私が寝るのを待っている？マルフタマルマル。提督、一体何をしているの？");
            voice_player_ptr->play();
            break;
        case 3:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/0300_ptr->mp3"));
            voice_label_ptr->setText("マルサンマルマル。この時間に食事をとると、人間は太るらしい。");
            voice_player_ptr->play();
            break;
        case 4:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/0400_ptr->mp3"));
            voice_label_ptr->setText("「おはよう」？それとも「おやすみ」？マルヨンマルマル。夜が一番深い時間。");
            voice_player_ptr->play();
            break;
        case 5:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/0500_ptr->mp3"));
            voice_label_ptr->setText("マルゴーマルマル。提督、私と一緒にコーヒーを飲む？");
            voice_player_ptr->play();
            break;
        case 6:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/0600_ptr->mp3"));
            voice_label_ptr->setText("マルロクマルマル。艦内照明を夜間モードから昼間モードに切り替える。");
            voice_player_ptr->play();
            break;
        case 7:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/0700_ptr->mp3"));
            voice_label_ptr->setText("マルナナマルマル。人間の営みが動き出す。今日の天気は、多分晴れ。");
            voice_player_ptr->play();
            break;
        case 8:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/0800_ptr->mp3"));
            voice_label_ptr->setText("マルハチマルマル。作戦行動開始。まずは、今日見るアニメの録画予約をセットして。");
            voice_player_ptr->play();
            break;
        case 9:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/0900_ptr->mp3"));
            voice_label_ptr->setText("マルキューマルマル。挨拶の言葉は、まだ「おはよう」？それとも「こんにちは」？");
            voice_player_ptr->play();
            break;
        case 10:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/1000_ptr->mp3"));
            voice_label_ptr->setText("ヒトマルマルマル。朝でも昼でもない中途半端な時間。中途半端って、何？");
            voice_player_ptr->play();
            break;
        case 11:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/1100_ptr->mp3"));
            voice_label_ptr->setText("午前のティータイム、ヒトヒトマルマル。私はコンゴウと違って、紅茶よりも甘いココアが好き。");
            voice_player_ptr->play();
            break;
        case 12:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/1200_ptr->mp3"));
            voice_label_ptr->setText("ヒトフタマルマル。ランチの時間、今日のメニューを教えて。");
            voice_player_ptr->play();
            break;
        case 13:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/1300_ptr->mp3"));
            voice_label_ptr->setText("ヒトサンマルマル。急速浮上して、甲板で日向ぼっこ。");
            voice_player_ptr->play();
            break;
        case 14:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/1400_ptr->mp3"));
            voice_label_ptr->setText("気持ちいい海風。このまま一日が過ぎていくのも悪くない…あ、ヒトヨンマルマル。");
            voice_player_ptr->play();
            break;
        case 15:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/1500_ptr->mp3"));
            voice_label_ptr->setText("甘いお菓子でちょっと休憩。ヒトゴーマルマルは、おやつの時間。");
            voice_player_ptr->play();
            break;
        case 16:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/1600_ptr->mp3"));
            voice_label_ptr->setText("ヒトロクマルマル。午後の4時。提督、もう一頑張りヨーソロー。");
            voice_player_ptr->play();
            break;
        case 17:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/1700_ptr->mp3"));
            voice_label_ptr->setText("ヒトナナマルマル。浮上して一緒に夕焼けを見てくれる？提督。");
            voice_player_ptr->play();
            break;
        case 18:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/1800_ptr->mp3"));
            voice_label_ptr->setText("ヒトハチマルマル。提督も手伝って。晩御飯の仕度を始める。");
            voice_player_ptr->play();
            break;
        case 19:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/1900_ptr->mp3"));
            voice_label_ptr->setText("ヒトキューマルマル。海の底から美味しそうな匂い。今夜のメニューは魚雷型コロッケ。");
            voice_player_ptr->play();
            break;
        case 20:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/2000_ptr->mp3"));
            voice_label_ptr->setText("フタマルマルマル。デザートは別腹。でも、メンタルモデルはいくら食べても太らない。");
            voice_player_ptr->play();
            break;
        case 21:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/2100_ptr->mp3"));
            voice_label_ptr->setText("フタヒトマルマル。提督が頭を洗ってくれるなら、お風呂に入ってもいいけど。");
            voice_player_ptr->play();
            break;
        case 22:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/2200_ptr->mp3"));
            voice_label_ptr->setText("フタフタマルマル。見ちゃダメ、提督。日記をつけてるんだから。もし見たら、全力でぶっ飛ばす。");
            voice_player_ptr->play();
            break;
        case 23:
            voice_player_ptr->setMedia(QUrl("qrc:/charater/voice/2300_ptr->mp3"));
